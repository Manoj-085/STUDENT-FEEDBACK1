...
function HomePage() {
  ...
  const initializeSampleFeedback = () => {
    // Initialize sample feedback data if not exists
  ...
  const handleSubmit = (e) => {
   ...
   // Corrected code in handleSubmit
const handleSubmit = (e) => {
    // ...
    const feedback = {
      id: Date.now(),
...
function FeedbackSummary({ currentUser }) {
  ...
  const myFeedback = allFeedback.filter(fb => fb.studentEmail === currentUser?.email);

  const totalRating = myFeedback.reduce((sum, fb) => sum + fb.rating, 0);
  const averageRating = myFeedback.length > 0 ? (totalRating / myFeedback.length).toFixed(1) : 0;
